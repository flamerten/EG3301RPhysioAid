import sys
import os
sys.path.append(os.getcwd()) #dangerous, it assumes u run simulate from the home directory
...
import socketio
import constants #From home directory
import time

from threading import Thread
from models import Reading #From home directory
...
def find_MCU_port():
    """Use this function of find the name of the port the M5 is connected to"""

    ports = list(serial.tools.list_ports.comports())
    
    for p in ports:
        print(p.description)
        if "M5" in p.description:
            print(p.device + " is the port" )
            return p.device
    
    print("Unable to find valid port, COM0 returned")
    return 'COM0' #m5 not found
...
def simulate(**kwargs):
    ...
    # num_of_sensors = int(os.getenv(constants.NUM_OF_SENSORS_ENV_VAR, constants.DEFAULT_NUM_OF_SENSORS)) << ??? Whhy is this here >>>
            ...
            try:
                ...
                while(mydata == ""):
                    ...
                    if(lis[0] != 'x') and (lis[-1]!='x'):
                        #Reset mydata to empty string to loop again, because data recieved is not valid
                        mydata = "" 
