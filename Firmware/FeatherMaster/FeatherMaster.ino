/*
 * MasterFeather -> Transmits Data to the M5. Vibration is optional
 * 
 */
...
uint8_t master[] = {0x4C,0x75,0x25,0xC4,0xFA,0xF4}; //Master M5 > calculate Joint angle and send it to the M5
...
unsigned const int BAT_TIME_DELAY_MILLIS =  60 * 1000; //Query every min
...
void CheckBattery(bool check_now){
    ...
    if( ( (millis() - last_bat_query_time) >= BAT_TIME_DELAY_MILLIS) || check_now){
        ...
        last_bat_query_time = millis(); //reset flag
    }
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
    ...
    data_ready = true; //flag to send updated data to the M5
}

void CheckToSend(){
  //Send to M5
  if(data_ready && ( (millis() - time_start_prog) >= STABALISE_DELAY)){
    //Use roll
    joint_angle = 180 - abs(180 - int(abs(slave_angle - roll)));
    //Set max flex of healthy person to be 0
    ...
    //Debug
    Serial.printf("Slave:%i Master:%i Joint:%i", slave_angle, roll, joint_angle);
    Serial.println();
...
void setup() {
  ...
  CheckBattery(true); //Just check for the first time and ignore the timing, check after the bat init
  ...
  time_start_prog = millis(); //start counting after init
