/*
 *  Testing Sketch with Ryan
 * Use of onboard to signify vibration
 *  
 */
...
uint8_t master[] = {0xE8,0x9F,0x6D,0x26,0x09,0x20}; //Master Feather
...
#define VIBRATE_TIME 300 //Vibrate for 1000 sec
...
void CheckVibrate(){
  ...
  if(start_vibrate){
    ...
    //testing using on board pixel
    if(!USEBAT){
      onboard_pixel.setPixelColor(0,onboard_pixel.Color(0,0,100));
      onboard_pixel.show();
    }
    ...
    //testing using on board pixel
    if(!USEBAT){
      onboard_pixel.setPixelColor(0,onboard_pixel.Color(0,0,0));
      onboard_pixel.show();
    }
  }
...
void Setup_DEEPSLEEP(){
  ...
  esp_sleep_enable_ext0_wakeup(GPIO_NUM_38,LOW); //change to 38 - inbuilt button
...
void get_readings(){
  ...
  //filter.updateIMU(gy, gx, -1.0*gz, ay, ax, -1.0 * az);
  filter.updateIMU(gz, gy, -1.0*gx, az, ay, -1.0*ax); //YAW
