...
int Time_RCV[] = {0,0,0}; //Use this to check the time last recieved
...
//Vibrate
uint8_t SlaveAddress[3][6] = { //CANNOT BE THE SAME
    {0xE8,0x9F,0x6D,0x25,0x3C,0x24}, //Slave 0
    {0xE8,0x9F,0x6D,0x1F,0xFD,0xB8}, //Slave 1 (testing unit)
...
void loop(){
    ...
    if(Serial.available()){//Check if data is available
        ...
        Angle_msg = RPI_msg.toInt(); //convert string to int

        if((Angle_msg >=0) && (Angle_msg <=2)){
            esp_now_send(SlaveAddress[Angle_msg], (uint8_t *) &message, sizeof(message));
        }
    ...
    if(button_press){
        ...
        esp_now_send(SlaveAddress[0], (uint8_t *) &message, sizeof(message)); //Send to 0
        delay(50);
    }
